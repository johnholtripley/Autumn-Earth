...
if ($uploadedfilesize > 102400) {
  ...
  for ($eachSong = 1; $eachSong<$numberOfSongs; $eachSong++) {
      ...
      while ($charIndex <= strlen($melody)) {
        ...
        if (stristr($validNotes, $baseNote) !== false) {
          ...
          while ((stristr($validNotes, $thisSubChar) === false) && ($subseqCharIndex <= strlen($melody))) {
            ...
            $divisor = 1;
				// check for division after this character:
				if (substr($melody, $subseqCharIndex+1, 1) == "/") {
					$divisor = substr($melody, $subseqCharIndex+2, 1);
					$subseqCharIndex += 2;
				}
            
              $thisNotesLength *= ($thisSubChar/$divisor);
            } else {
              switch ($thisSubChar) {
	...
	header('Content-Disposition: attachment; filename="'.$songTitle.'.wav"');
...
<form action="test.php" name="abcForm" id="abcForm" enctype="multipart/form-data" method="post">
<fieldset>
